...
test('Technik › Pipeline explains every stage interactively and traces a contract', async ({ page }) => {
  await page.goto('/tech/pipeline')
  await expect(page.getByRole('heading', { name: 'So arbeitet die Pipeline' })).toBeVisible()
  await expect(page.getByRole('img', { name: 'So arbeitet die Pipeline' })).toBeVisible()
  await page.getByTestId('stage-verify').click()
  await expect(page.getByRole('heading', { name: 'KI-Gegenprüfung' })).toBeVisible()
  await expect(page.getByText('Was passiert')).toBeVisible()
  await expect(page.getByText(/gemini-3\.1-pro-preview/).first()).toBeVisible()
  await expect(page.getByText('Code: api/app/audits/verify.py')).toBeVisible()
  await page.getByTestId('stage-gate').click()
  await expect(page.getByText(/80 % Eskalation/)).toBeVisible()
  await page.getByLabel('Vertrag verfolgen').click()
  await page.getByRole('option', { name: /C10_merchant_agreement_lumen_mixed|Merchant Agreement/ }).first().click()
  await expect(page.getByText(/^Für /)).toBeVisible()
  await expect(page.getByText(/Seite 1: /).first()).toBeVisible()
  ...
  await page.getByTestId('stage-verify').click()
  await page.screenshot({ path: 'e2e/screenshots/10-pipeline.png', fullPage: true })
})
