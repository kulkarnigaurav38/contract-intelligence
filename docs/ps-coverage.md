...
| --- | --- | --- | --- |
| Azure | ✅ (IaC) | Terraform: Container Apps, Neo4j (AuraDB from the marketplace, or the container), Blob, Key Vault, Document Intelligence, Log Analytics, Entra ID group access | — |
...
| Database, index and knowledge graph (not named) | ✅ Neo4j (`api/app/db.py`, `api/app/graph.py`) | one graph for documents, pages, clauses (vector + full-text index), name mentions, guideline as `REQUIRES` edges, name register from GLEIF with `RENAMED_TO`, audits, findings, decisions and the audit log; the verifier and the drafter read what the graph selects, an absence is confirmed against the full contract; `GET /api/graph/gaps` is the cross-contract question as one pattern | Apache AGE + pgvector on Azure Database for PostgreSQL (GA) or Cosmos DB for Gremlin — see `docs/tech-choices.md` §3 |
...
| Basic front-end demonstrating how the solution can be used | `web/` — German-first legal-team UI in three steps (Hochladen · Prüfen · Herunterladen): drop zone → automatic result → filter row for the cross-contract question → the contract with its findings marked on the pages and one finding at a time to decide → the download page with the corrected copy and its preview (browser tests in `web/e2e/smoke.spec.ts`) | ✅ |
...
| Presentation: solution, technical solution, infrastructure, adoption concept, other | `docs/presentation.html` — 15 slides generated by `docs/deck/build.py` (problem · estimation · thought process · versions · workflow · read · graph · check · decide · UI · infrastructure · alignment · adoption on slide 14 · demo); the same slides as a design canvas under `docs/deck/artboards/` | ✅ |
| Explainability, technical | *Technik* page (`web/src/pages/Technik.tsx`, generated from `GET /api/pipeline` + `TECH` in `api/app/pipeline.py`, `GET /api/graph/stats`, `GET /api/documents/{id}`): every stage with module, model, thinking level, rules and thresholds; the graph with live counts and the gap query; a trace of one contract through all stages | ✅ |
...
automatic per-contract report uses the same rule layer and the same verifier (`report.py` calls
...
### D2. Test suites
The end-to-end suite runs against a scratch Neo4j (`docker compose --profile test up -d neo4j-test`, `NEO4J_TEST_URI=bolt://localhost:7688`); on 2026-09-05 it passed with the graph as the only store.
...
end-to-end tests on the scratch Neo4j (`tests/test_end_to_end_graph.py`, wipes that graph first) and the 10
